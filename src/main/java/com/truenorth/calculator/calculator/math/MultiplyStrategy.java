...
public class MultiplyStrategy implements CalculatorStrategy {
    ...
    public OperationResultDTO calculate(MathRequestDTO mathRequestDTO) {
        return OperationResultDTO.builder().result(mathRequestDTO.getValue1().multiply(mathRequestDTO.getValue2())).build();
    }

    @Override
    public CalculatorStrategyName getStrategyName() {
        return CalculatorStrategyName.MultiplyStrategy;
    }
}
