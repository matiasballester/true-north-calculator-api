...
public class SubtractStrategy implements CalculatorStrategy {
    ...
    public OperationResultDTO calculate(MathRequestDTO mathRequestDTO) {
        return OperationResultDTO.builder().result(mathRequestDTO.getValue1().subtract(mathRequestDTO.getValue2())).build();
    }

    @Override
    public CalculatorStrategyName getStrategyName() {
        return CalculatorStrategyName.SubtractStrategy;
    }
}
